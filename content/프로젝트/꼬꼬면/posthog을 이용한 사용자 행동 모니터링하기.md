...
가장 먼저 도입했던 것은 `Sentry`였다. Sentry는 에러 추적에 용이한 로그 관리를 주로 해주었기 때문에 확실히 에러 추적에는 편했다. 또한 다른 사용자 행동 모니터링에 대한 기능 또한 제공하는데, 문제는 다음과 같았다:
...
그렇기 때문에 에러 로깅은 Sentry로, 사용자 행동에 대한 모니터링은 다른 오픈소스를 통해서 관리하려고 했고, 이번에 선택한 오픈소스가 바로 `PostHog`이다.
