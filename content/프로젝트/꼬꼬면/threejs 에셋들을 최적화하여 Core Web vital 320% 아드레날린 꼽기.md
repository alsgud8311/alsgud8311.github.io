...
저희가 기존보다 어떤 방식으로 서비스를 개선하려고 했는지, 그 과정에서 web vital이 25까지 떨어진 상황에서 80점까지 올리며 약 320% 정도를 개선했던 과정을 이야기해보려고 합니다. 
## 문제의 발단
...
마지막 단계에서 다시 GLB로 변환하는 이유는 바이너리 데이터의 효율성 때문입니다. 8비트 바이너리 데이터가 Base64(6비트 기준)로 인코딩되면 데이터 크기가 약 33% 증가하게 되는데, 이는 웹 환경에서 치명적인 성능 저하를 초래할 수 있기 때문입니다.

또한 GLTF는 JSON 형식으로 되어 있기 때문에 해당 파일을 파싱하고, GLTF 안에 선언되어 있는 텍스처와 같은 외부 리소스를 가져오는 과정 또한 있기 때문에, 이로 인한 네트워크 비용 증가와 로딩 속도 증가 또한 불가피한 점이다 보니 하나의 파일로 가져오는 것이 더 효율적이겠죠?

따라서 저는 GLB -> GLTF -> GLB의 과정을 통해 용량을 최대한 줄이려 했습니다. 현재는 일단 먼저 gltf 파일로 blender에서 변환을 진행했어요.
...
GLTF로 변환된 모델 데이터를 다시금 GLB로 blender에서 export 하면서 최적화 옵션을 통해 추가적으로 최적화를 진행했고, 1.8MB -> 1.3MB로 다시금 약 70%의 용량으로 줄일 수 있었습니다.

## 정리
이번 이야기에서는 3D 모델을 도입하면서 performance가 25정도까지 떨어지던 문제를 해결하기 위해 최적화를 진행했던 과정들을 담았습니다.

최적화를 진행했던 부분들은 3D 에셋들에 대해서 
- 텍스처 이미지 최적화
- gltfpack을 이용한 모델 데이터 최적화(메시, 애니메이션 키프레임 등)
- 바이너리 파일로 재변환하면서 최적화
등의 방법을 통해 최적화를 진행하였고, 처음 8.3MB였던 glb 파일 크기를 1.3MB까지 약 85% 용량을 개선할 수 있었으며, Web vital 지표로 보면 Performance를 25에서 약 80까지 거의 320%를 개선할 수 있었어요. 
![](https://i.imgur.com/eYb8xu9.png)
심지어 현재는 아직 glb 파일은 cdn에 올려두지 않은 상태로 gltf 최적화까지만 진행했음에도 같은 인터페이스를 제공하면서 성능은 놀랄 정도로 개선되었습니다. glb 파일은 아직 프로덕션까지는 올라가지 않았지만 해당 파일이 올라가면 또 얼마나 개선되었을까 두근두근 하기도 하네요ㅎㅎ

추가적으로 해당 3D 에셋과 랜딩 페이지 개선 작업을 통해 기존에는 45%였던 bounce rate를 38% 정도까지 개선할 수 있었습니다. 실험적으로 도입했던 요소들이었지만 좋은 효과를 내서 만족스럽네요😊

![](https://i.imgur.com/uwluz1k.png)

만약 여러분들도 threejs와 같은 3d 에셋을 이용한 라이브러리를 사용할 때, 가장 크게 신경쓰는 부분이 이러한 에셋들의 크기일 것이라고 생각합니다. 저와 같은 방법으로 최적화를 진행해보고 많이 성능 개선을 이루셨으면 하는 마음으로 글을 마칩니다.