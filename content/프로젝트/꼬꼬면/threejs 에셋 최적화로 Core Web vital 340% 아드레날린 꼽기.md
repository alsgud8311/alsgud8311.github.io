...
마지막 단계에서 다시 GLB로 변환하는 이유는 바이너리 데이터의 효율성 때문입니다. 8비트 바이너리 데이터가 Base64(6비트 기준)로 인코딩되면 데이터 크기가 약 33% 증가하게 되는데, 이는 웹 환경에서 성능 저하를 초래할 수 있기 때문입니다.

또한 GLTF는 JSON 형식으로 되어 있기 때문에 해당 파일을 파싱하고, GLTF 안에 선언되어 있는 텍스처와 같은 외부 리소스를 가져오는 과정 또한 있기 때문에, 이로 인한 네트워크 비용 증가 또한 불가피한 점이다 보니 하나의 파일로 가져오는 것이 더 효율적이겠죠?
...
- 바이너리 파일로 재변환하면서 최적화
등의 방법을 통해 최적화를 진행하였고, 처음 8.3MB였던 glb 파일 크기를 1.3MB까지 약 85% 용량을 개선할 수 있었으며, Web vital 지표로 보면 Performance를 25에서 약 86까지 거의 약 340%를 개선할 수 있었습니다.
...
추가적으로 해당 3D 에셋과 랜딩 페이지 개선 작업을 통해 기존에는 45%였던 bounce rate를 38% 정도까지 개선할 수 있었습니다. 실험적으로 도입했던 요소들이었지만 좋은 효과를 내서 만족스럽습니다😊
